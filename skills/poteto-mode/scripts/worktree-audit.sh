...
# Transcripts: ~/.claude/projects/<slug>/*.jsonl, where <slug> is the workspace
# path with every "/" and "." turned into "-". A chat run inside a worktree gets
# its own slug dir, so scan the repo's dir plus every dir prefixed by it. Still
# workspace-scoped: unrelated projects are never read.
slug=$(printf '%s' "$main_wt" | tr '/.' '-')
transcript_dirs=()
for d in "$HOME/.claude/projects/$slug" "$HOME/.claude/projects/$slug"-*; do
...
git worktree list --porcelain | awk '/^worktree /{print $2}' | while read -r wt; do
	...
	if [ ${#transcript_dirs[@]} -gt 0 ]; then
		f=$(rg -l -e "${wt}/" -e "${wt}\"" "${transcript_dirs[@]}" 2>/dev/null \
			| xargs stat -f '%m %N' 2>/dev/null | sort -rn | head -1)
		[ -n "$f" ] && last_ts=$(echo "$f" | awk '{print $1}')
	fi
	# A chat whose cwd was the worktree itself has its own slug dir; its newest
	# transcript is a stronger signal than a path mention.
	wt_dir="$HOME/.claude/projects/$(printf '%s' "$wt" | tr '/.' '-')"
	if [ -d "$wt_dir" ]; then
		wt_ts=$(find "$wt_dir" -name '*.jsonl' -exec stat -f '%m' {} + 2>/dev/null | sort -rn | head -1)
		[ -n "$wt_ts" ] && [ "$wt_ts" -gt "$last_ts" ] 2>/dev/null && last_ts=$wt_ts
	fi
	[ "$last_ts" -gt 0 ] 2>/dev/null && last=$(date -r "$last_ts" '+%Y-%m-%d' 2>/dev/null)
