...
- **Worker / verifier.** A background subagent, given `isolation: "worktree"` whenever it writes code, so two workers never share a tree. Reach for `isolation: "remote"` only where remote execution is enabled and the unit needs no local state; anything touching this machine (runtime verification through a `verify-*` skill, local transcripts, simulators, auth that exists only here) stays in the local tree. A remote worker cannot read the local store, so its brief inlines what it needs or points at repo paths. Prefer fewer, broader workers; one writer per worktree or branch (principle-separate-before-serializing-shared-state). Run a unit's verifier on a different model family from its worker.
...
A sub-coordinator brief adds its track boundary and unit list, its spawn budget with the isolation default and the exception list, the drain protocol, and the rollup format (per child: name, status, PR, head SHA, verdict, one line; plus track status and frontier delta).
...
- Exactly one stacker per stack may run `gt`, serialized within its stack; record the holder in the standing orders. Run restacks in a dedicated worktree; a restack over the main tree at this scale takes the laptop down.
...
- Never resume an agent to check on it; a resume restarts an idle agent. Probe read-only: the ledger, `units.tsv`, `gh`, pushed branches, and `ListAgents` or `claude agents` for a session's status. Transcript mtime is not liveness.
