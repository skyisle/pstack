...
**Control skill.** Pick it by surface. The repo's own `verify-*` skill wins when it has one (generate it with `/pstack:create-verification-skill`). Otherwise: browser, Electron, and web UIs use `/webapp-testing` or `/agent-browser`; CLIs, TUIs, and servers use `/run`; native mobile uses `/smoke` or whatever simulator-driving skill the repo has. A PR that touches two surfaces gets lanes on both. A surface with no control skill is a risk in Appendix C, and its live block still names how each lane drives it.
...
- [ ] Run `/simplify` before each commit and `/pstack:no-comments` before review.
...
Each live lane runs in its own worktree at the PR head, or on a remote-isolated agent where remote execution is enabled. Drive through the control skill picked above.
