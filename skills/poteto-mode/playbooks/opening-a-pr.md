...
- `## Verification`. State how you ran each check and its rigor. Name the real path, such as the repo verify-* skill, `/webapp-testing`, `/smoke`, or the targeted tests. State the outcome of each check, not only the command name.
...
A subagent that opens a PR runs `interrogate`, `/simplify`, and `/pstack:no-comments`. It returns the URL and does not babysit. Return to the parent.
