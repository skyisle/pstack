...
The parent finds its own transcript file before fanning out. This session's transcript is `~/.claude/projects/<slug>/<session-id>.jsonl`, where `<slug>` is this workspace's absolute path with every `/` and `.` turned into `-` (so `/Users/you/proj` becomes `-Users-you-proj`). The system prompt's scratchpad path ends in `<session-id>/scratchpad`, so it names the session id directly. Stay inside this workspace's directory; do not glob across `~/.claude/projects/*/`, which crosses workspace boundaries and reads private chats from unrelated projects.
...
Two layouts: the session itself (`<session-id>.jsonl`) and its subagents (`<session-id>/subagents/**/agent-*.jsonl`, marked `"isSidechain":true`).

For each candidate, read the first user record and check that `message.content` contains the conversation's opening user prompt. Take the matching path. If no path resolves, write a tight digest of the session and pass that instead.
