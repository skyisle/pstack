...
Transcripts live under `~/.claude/projects/<slug>/`, where `<slug>` is this workspace's absolute path with every `/` and `.` turned into `-` (so `/Users/you/proj` becomes `-Users-you-proj`). One `.jsonl` per chat, named by session id; subagent transcripts sit beside it in `<session-id>/subagents/**/*.jsonl`. Every line is one JSON record, most of them chat messages (`type` is `user` or `assistant`); non-message records like `mode` and `file-history-snapshot` are bookkeeping and can be filtered out with `jq`. When the directory name is not obvious, list `~/.claude/projects/` and match the path by eye.
