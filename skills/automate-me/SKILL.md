...
Locate the active workspace's transcripts before fanning out: `~/.claude/projects/<slug>/*.jsonl`, where `<slug>` is this workspace's absolute path with every `/` and `.` turned into `-`. Use only that directory. Don't glob across `~/.claude/projects/*/`. That crosses workspace boundaries and reads private chats from unrelated projects.
